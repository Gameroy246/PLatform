  ...
  return (
    <div
      style={{
        ...
        border: selected ? `2px solid ${accentColor}` : data.status === 'SUCCESS' ? '2px solid #4ade80' : data.status === 'ERROR' ? '2px solid #f87171' : data.status === 'RUNNING' ? `2px solid ${accentColor}` : `1px solid ${isMissingFile ? '#ef4444' : 'var(--border)'}`,
        borderRadius: 8,
        padding: '16px',
        minWidth: 220,
